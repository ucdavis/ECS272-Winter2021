        ...
        d3.csv(this.props.data)
            .then(csv => {
                ...
                for(i = 0; i < slice.length; i ++){
                    var yValue = 30 + i*30;
                    svg.append("circle").attr("cx",50).attr("cy",yValue).attr("r", 6).style("fill", color(slice[i]))
                    svg.append("text").attr("x", 70).attr("y", yValue).text(slice[i]).style("font-size", "15px").attr("alignment-baseline","middle")
                }
